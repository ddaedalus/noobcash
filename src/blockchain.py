...
class Blockchain():
    ...
    def set_copy_params(self, addresses,
                        identity):  # set params representing that this chain is representative of a specific node
        self.id = identity
    ...
    def add_trans(self, trans):
        ...
        if (len(self.list_of_trans) == CHAIN_CAPACITY):  # max capacity reached so create new block, do mine
            ...
            trans_dict = [json.loads(i.transaction_to_json()) for i in
                          self.list_of_trans]  # list of jsons for all trans that are going to be in the block
            prev_hash = self.list_of_blocks[
                -1].cur_hash  # get hash of last block in order to use it as prev hash for the next block
    ...
    def mine_job(self, new_bl):
        ...
        if (not self.miner.isSet() and node.consensus.isSet() is False):
            self.list_of_blocks.append(new_bl)
            fd = open('res/time_to_mine.txt', 'a')  # file for mine times for each node
            fd.write(str(time.time() - float(strart_mine_time)) + '\n')  # write mine time in file and close
            fd.close()
            print('Mined block')  # , time.time(),threading.enumerate())
            start_time = time.time()  # when we mined the block
            node.no_mine.set()
            ...
            for adr in self.ring:  # found valid nonce broadcast new block to all except u
                if (adr != self.ring[self.id]):
                    mes = {'last_block': self.list_of_blocks[-1].block_to_json(),
                           'mine_time': start_time}  # message that sends last block in json format
                    final_msg = adr + '/all/mined_block'
                    resp = requests.post(final_msg, json=mes, headers=headers)
